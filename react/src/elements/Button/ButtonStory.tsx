...
export function ButtonStory(_props: ButtonProps) {
  const { label, icon, id, tooltip, variant = ButtonVariant.STANDARD, ...props } = _props;

  return (
    <div css={css({ overflow: "auto", height: "100%" })}>
      {styleList.filter((buttonStyle) => {
        if (variant === ButtonVariant.TEXT) {
          return buttonStyle === "outline";
        }
        return true;
      }).map((buttonStyle) => (
        <div key={buttonStyle} css={css({ borderStyle: "solid", borderWidth: "1px 0px", paddingBottom: "1rem"})}>
          <h1>{startCase(buttonStyle)}</h1>
          {buttonVariations.map(({id: varID, label: varLabel, ...btnProps}) => (
            <div key={varID} css={css({ margin: "0 1rem", paddingBottom: "1rem"})}>
              <h2>{varLabel}</h2>
              <div css={css({ display: "flex", flexWrap: "wrap", margin: "0 1rem" })}>
                {colorKeys.map((colorKey) => (
                  <Button
                    ...
                    tooltip={tooltip ? `${tooltip}-${startCase(colorKey)}` : undefined}
                    variant={variant}
                    colorKey={colorKey}
